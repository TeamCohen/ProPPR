 ...
 *
 */
public class TroveGraph {
	private static final Logger log = Logger.getLogger(TroveGraph.class);
	...
	private static final TIntDoubleHashMap EMPTY_MAP = new TIntDoubleHashMap();
	protected TIntObjectMap<TIntDoubleHashMap> near = new TIntObjectHashMap<TIntDoubleHashMap>();
	protected TIntDoubleMap total = new TIntDoubleHashMap();
	protected int numEdges=0;
	
	protected TObjectIntMap<String> keyToId=new TObjectIntHashMap<String>();
	protected TIntObjectMap<String> idToKey=new TIntObjectHashMap<String>();
	...
	public int[] keyArray(int length) {
		return new int[length];
	}
	public static int nextIdPeek() { return nextKey.intValue(); }
	
	public int[] keyToId(String[] key) {
		int[] result = keyArray(key.length);
...
//	}
	 ...
	 */
	public TIntDoubleHashMap rwr(TIntDoubleHashMap startVec) {
	 ...
	 * @return String->Double map of graph nodes serving as the rwr result
	 */
	public TIntDoubleHashMap rwr(TIntDoubleHashMap startVec, double gamma, int maxT) {
		TIntDoubleHashMap vec = startVec;
	 ...
	 */
	public TIntDoubleHashMap walkOnce(TIntDoubleHashMap vec0,TIntDoubleHashMap vec,double gamma) {
	 ...
	 * @return String->Double map of next vector
	 */
	public TIntDoubleHashMap walkOnce(TIntDoubleHashMap vec0,TIntDoubleHashMap vec,double gamma,double interval, String msg) {
		TIntDoubleHashMap nextVec = new TIntDoubleHashMap();
		for (TIntDoubleIterator e = vec0.iterator(); e.hasNext(); ) {
			e.advance();
			nextVec.adjustOrPutValue(e.key(), gamma * e.value(), 0);
	//				Dictionary.increment(nextVec,key,gamma * value);
		}
		int k=0;
		for(TIntDoubleIterator v = vec.iterator(); v.hasNext(); ) { 
			v.advance();
			k++; if ( (k%interval) == 0) System.err.println(String.format(msg,k));
//			K v = e.getKey();
			if (near.containsKey(v.key())) {
				double z = total.get(v.key());
				for (TIntDoubleIterator item = near.get(v.key()).iterator(); item.hasNext();) {
					item.advance();
					int u = item.key();
					double w_vu = item.value();
					Dictionary.increment(nextVec, u, (1.0 - gamma) * v.value() * w_vu / z);
				}
			}
		}
		return nextVec;
	 ...
	 */
	public TIntDoubleHashMap near(int u) {
		TIntDoubleHashMap ret = near.get(u);
		if (ret==null) return EMPTY_MAP;
		return ret;
	}
	 ...
	 */
	public TIntSet getNodes() { return near.keySet(); }
	
	public Set<String> getNodesAsStrings() {
		TreeSet<String> result = new TreeSet<String>();
		for (TIntIterator id = getNodes().iterator(); id.hasNext(); ) {
			result.add(idToKey(id.next()));
		}
		return result;
	}
