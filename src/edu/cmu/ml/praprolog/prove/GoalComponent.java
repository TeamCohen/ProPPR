...
public class GoalComponent extends Component {
    ...
    public void addFact(Goal goal) {
        if (goal.getArity() < 1)
            throw new IllegalArgumentException("Goal arity must be >= 1 for goal " + goal.toString());
        for (int ai = 0; ai < goal.getArgs().length; ai++) {
            if (!goal.getArg(ai).isConstant())
                throw new IllegalArgumentException("Goal arguments must all be constant for goal " +
                                                   goal.toString() + ", goal " + ai + " isn't");
        }
    ...
    public List<Outlink> outlinks(LogicProgramState state) {
        ...
        for (Goal g : this.goalsMatching(state.getHeadFunctor(), state.getHeadArity(), state.getHeadArg1())) {
            if (log.isDebugEnabled())
                log.debug("trying goal " + g + " on " + state.getHeadGoal() + " with offset " + 0);
    ...
    public Iterable<Goal> goalsMatching(Goal goal) {
    ...
    public static GoalComponent loadCompiled(List<String> files) {
        GoalComponent result = new GoalComponent(files.get(0) +
                                                 (files.size() > 1 ? "+" + (files.size() - 1) + "others" : ""));
