...
public class Configuration {
    ...
    protected void retrieveSettings(CommandLine line, int flags, Options options) {
        ...
        if (isOn(flags, USE_COMPLEX_FEATURES) && line.hasOption("complexfeat_config"))
            this.complexFeatureConfigFile = line.getOptionValue("complexfeat_config");
    ...
    protected void addOptions(Options options, int flags) {
        ...
        options.addOption(
                OptionBuilder
                        .withLongOpt("complexfeat_config")
                        .withArgName("cf")
                        .hasArgs()
                        .withDescription("Path to (simple) configuration file for complex features.")
                        .create());
    }
