...
public class TestNeqPlugin {
	...
	public void test() throws IOException, LogicProgramException {
		...
		assertEquals("different should have 1 solution",1,p.solutions(new ProofGraph(different,apr,program)).size());
		assertEquals("same should have no solution",0,p.solutions(new ProofGraph(same,apr,program)).size());
	}
