...
import edu.cmu.ml.proppr.graph.v1.AnnotatedGraph.GraphFormatException;
...
//		totalNegLossThisEpoch = 0;
	...
	public static void main(String[] args) {
		...
		if (c.trove) {
			edu.cmu.ml.proppr.trove.Trainer trainer = (edu.cmu.ml.proppr.trove.Trainer) c.trainer;
			paramVec = trainer.trainParametersOnCookedIterator(
					new edu.cmu.ml.proppr.trove.learn.tools.CookedExampleStreamer(cookedFile), 
					c.epochs, 
					c.traceLosses);
		} else {
			Trainer<String> trainer = (Trainer<String>) c.trainer;
			paramVec = trainer.trainParametersOnCookedIterator(
					new GroundedExampleStreamer<String>(cookedFile, new AnnotatedGraphFactory<String>(AnnotatedGraphFactory.STRING)),
					c.epochs,
					c.traceLosses);
		}
