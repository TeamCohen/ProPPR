...
import edu.cmu.ml.proppr.trove.MultithreadedRRTrainer;
import edu.cmu.ml.proppr.trove.MultithreadedTrainer;
import edu.cmu.ml.proppr.trove.Trainer;
import edu.cmu.ml.proppr.trove.learn.L2PosNegLossTrainedSRW;
...
public class ExperimentConfiguration extends Configuration {
	...
	private void troveSeed(CommandLine line) {
		if (!line.hasOption("seed")) return;
		long seed = Long.parseLong(line.getOptionValue("seed"));
		edu.cmu.ml.proppr.trove.learn.SRW.seed(seed);
	}
	...
	protected void setupSRW(CommandLine line, int flags, Options options) {
		...
		if (line.hasOption("srw")) {
			...
			if (values[0].equals("l2p")) {
				if (this.trove) {
					this.srw = new edu.cmu.ml.proppr.trove.learn.L2PosNegLossTrainedSRW(maxT,mu,eta,weightingScheme,delta);
				} else {
				...
				if (this.trove) {
					this.srw = new edu.cmu.ml.proppr.trove.learn.LocalL2PosNegLossTrainedSRW(maxT,mu,eta,weightingScheme,delta);
				} else {
				...
				if (this.trove) {
					this.srw = new edu.cmu.ml.proppr.trove.learn.AprSRW(maxT, mu, eta, weightingScheme, delta, alpha, epsilon, AprSRW.DEFAULT_STAYPROB);
				} else {
